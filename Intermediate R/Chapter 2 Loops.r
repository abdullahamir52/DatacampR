...
# Loops can come in handy on numerous occasions. While loops are like repeated
# if statements, the for loop is designed to iterate over all elements in a
# sequence. Learn about them in this chapter.
...
# Have a look at the code on the right; it initializes the speed variables and
# already provides a while loop template to get you started.
...
# this new value to speed again. This step is crucial; otherwise your while
# loop will never stop and your session will expire.
...
# printed out to the console, resulting in a decrease of your speed by 7 units.

# There are several ways in which you could make your driver's assistant more
# advanced. For example, the assistant could give you different messages based
# on your speed or provide you with a current speed at a given moment.
...
# there's no code that decreases the speed variable yet, which is pretty
# dangerous. Can you make the appropriate changes?
...
# what if a hurricane is approaching and you have to get away as quickly as
# possible? You don't want the driver's assistant sending you speeding
# notifications in that scenario, right?
...
# Adapt the while loop such that it is abandoned when the speed of the vehicle
# is greater than 80. This time, the speed variable has been initialized to 88;
# keep it that way.
...
# comprehensive exercise using a while loop, you're totally ready for the
# next topic: the for loop.
...
# Remember our linkedin vector? It's a vector that contains the number of
# views your LinkedIn profile had in the last seven days. The linkedin vector
# has already been defined in the editor on the right so that you can fully
...
# its population size, the names of the boroughs, and whether it is the capital
# of the United States. We've already prepared a list nyc with all this
# information in the editor (source: Wikipedia).
...
# tic-tac-toe game. It contains the values "X", "O" and "NA". Print out ttt in
# the console so you can have a closer look. On row 1 and column 1, there's
# "O", while on row 3 and column 2 there's "NA".

# To solve this exercise, you'll need a for loop inside a for loop, often
# called a nested loop. Doing this in R is a breeze! Simply use the following
# recipe:
...
# information in the following format: "On row i and column j the board
# contains x", where x is the value on that position.
...
# In the editor on the right you'll find a possible solution to the previous
# exercise. The code loops over the linkedin vector and prints out different
# messages depending on the values of li.

# In this exercise, you will use the break and next statements:

# The break statement abandons the active loop: the remaining code in the loop
# is skipped and the loop is not iterated over anymore.
...
# rquote. This variable has been split up into a vector that contains separate
# letters and has been stored in a vector chars with the strsplit() function.
...
# Finally, print out the variable rcount to the console to see if your code
# is correct.
