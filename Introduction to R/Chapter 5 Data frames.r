...
# You may remember from the chapter about matrices that all the elements that 
...
# above, is a data set of different data types. You will often find yourself
# working with data sets that contain different data types instead of only one.
...
# with (extremely) large data sets and data frames, your first task as a data 
# analyst is to develop a clear understanding of its structure and main 
# elements. Therefore, it is often useful to show only a small part of 
# the entire data set.

# So how to do this in R? Well, the function head() enables you to show the 
# first observations of a data frame. Similarly, the function tail() prints 
# out the last observations in your data set.
...
# Call head() on the mtcars data set to have a look at the header and 
...
# the function str(). The function str() shows you the structure of your 
# data set. For a data frame it tells you:
...
# The first observations
# Applying the str() function will often be the first thing that you do 
# when receiving a new data set or data frame. It is a great way to get 
# more insight in your data set before diving into the real analysis.

# Investigate the structure of mtcars. Make sure that you see the same 
# numbers, variables and data types as mentioned above.
...
# Since using built-in data sets is not even half the fun of creating your 
...
# Use the function data.frame() to construct a data frame. Pass the vectors 
# name, type, diameter, rotation and rings as arguments to data.frame(), 
...
# The planets_df data frame should have 8 observations and 5 variables. It 
# has been made available in the workspace, so you can directly use it.
...
# the help of square brackets [ ]. By using a comma, you can indicate what to 
# select from the rows and the columns respectively. For example:
 
# my_df[1,2] selects the value at the first row and second column in my_df.
# my_df[1:3,2:4] selects rows 1, 2, 3 and columns 2, 3, 4 in my_df.
# Sometimes you want to select all elements of a row or column. For example, 
# my_df[1, ] selects all elements of the first row. Let us now apply this 
# technique on planets_df!

# From planets_df, select the diameter of Mercury: this is the value at the 
# first row and the third column. Simply print out the result.
# From planets_df, select all data on Mars (the fourth row). Simply print 
# out the result.
...
# from a data frame. If you want to select all elements of the variable 
# diameter, for example, both of these will do the trick:
...
# Use the $ sign to select the rings variable from planets_df. Store the 
# vector that results as rings_vector.
...
# You probably remember from high school that some planets in our solar system 
# have rings and others do not. Unfortunately you can not recall their names. 
...
# ring (FALSE), but the other four do (TRUE). However, you do not get a nice 
# overview of the names of these planets, their diameter, etc. Let's try to 
...
# Adapt the code so that instead of only the name column, all columns for 
# planets that have rings are selected.
...
# condition was true (rings or no rings), and you managed to pull out all 
# relevant data. Pretty awesome! By now, NASA is probably already flirting 
# with your CV ;-).

# Now, let us move up one level and use the function subset(). You should 
# see the subset() function as a short-cut to do exactly the same as what 
# you did in the previous exercises.

# subset(my_df, subset = some_condition)
# The first argument of subset() specifies the data set for which you want 
...
# Use subset() on planets_df to select planets that have a diameter smaller 
# than Earth. Because the diameter variable is a relative measure of the 
...
# Experiment with the order() function in the console. Click 'Submit Answer' 
# when you are ready to continue.

b<-c(1,3,2,5,9,7,4,2,1)
order(b)
# [1] 1 9 3 8 2 7 4 6 5
b[order(b)]
# [1] 1 1 2 2 3 4 5 7 9
...
# useful with it. You would like to rearrange your data frame such that it 
# starts with the smallest planet and ends with the largest one. A sort on 
# the diameter column.

# Call order() on planets_df$diameter (the diameter column of planets_df). 
# Store the result as positions.

# Now reshuffle planets_df with the positions vector as row indexes inside 
# square brackets. Keep all columns. Simply print out the result.
